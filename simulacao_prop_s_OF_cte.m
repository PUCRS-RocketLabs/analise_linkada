...
%motor
dgrao=65/1000;            %diametro do grao combustivel
d_port=30/1000;
egrao=(dgrao-d_port)/2;
hgrao=130/1000;
ngrao=2;
moxt=3.8;              %quantidade de oxidante no tanque
F=1500;
% Pa (730 psi)
...
% figure(2)
subplot(2,3,5)
plot(ts,mfs*1000+moxs*1000,'b','LineWidth',1.5)
title('Vazão mássica')
grid minor
xlabel('tempo (s)')
ylabel('Vazão mássica total (g/s)')
...
type(k) = menu('Deseja simular o desempenho?','Sim',...
        'Não');
    if type==1
        run desempenho_foguete_balistico.m
    end